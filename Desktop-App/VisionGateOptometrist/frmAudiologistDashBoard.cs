...
{
    public partial class frmAudiologistDashBoard : Form
    {
        ...
        public frmAudiologistDashBoard()
        {
            InitializeComponent();
        }

        private void frmAudiologistDashBoard_Load(object sender, EventArgs e)
        {
        ...
        private void LoadEmployeeName()
        {
            try
            {
                ...
                {
                    con.Open();
                    // Query to fetch the name of the employee with EmployeeID = 7
                    string query = "SELECT Name FROM tblManageEmployee WHERE EmployeeID = 7";
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        var result = cmd.ExecuteScalar(); // Execute the query and get a single result
                        if (result != null)
                        {
                            lblName.Text = result.ToString(); // Display the name in lblName
                        }
                        else
                        {
                            lblName.Text = "Name Not Found"; // Set default text if no data is found
                        }
                    }
                }
            }
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            ...
                            adapter.Fill(notificationTable); // Populate the DataTable
                            dataGridView1.DataSource = notificationTable; // Set the DataGridView's data source
