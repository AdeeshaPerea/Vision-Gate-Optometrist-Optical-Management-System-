...
{
    public partial class frmViewHearingTestResults : Form
    {
        private string connectionString = @"Data Source=LAPTOP-S6UOBFRN\SQLEXPRESS; Initial Catalog=OSMS; Integrated Security=True";

        public frmViewHearingTestResults()
        {
            InitializeComponent();
        }

        private void frmViewHearingTestResults_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.None;
            LoadHearingTestResults();
        }

        private void LoadHearingTestResults()
        {
            try
            {
                ...
                {
                    con.Open();
                    string query = "SELECT * FROM tblManageHearingTestResults";
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            DataTable hearingTestResultsTable = new DataTable();
                            adapter.Fill(hearingTestResultsTable);

                            dataGridView1.DataSource = hearingTestResultsTable;
            ...
            {
                MessageBox.Show($"Error loading hearing test results: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
